...
import (
	...
	oukvStore "github.com/i5heu/ouroboros-kv/internal/store"
...
const (
  ...
  %s -lsx                      List all stored data with extended slice statistics
...
Examples:
  ...
  %s -lsx                      # List with Reed-Solomon slice statistics
...
func initKV() (*oukvStore.KV, error) {
	...
	// Initialize KV
	kv, err := oukvStore.Init(cryptInstance, config)
	if err != nil {
...
func storeFile(kv *oukvStore.KV, filePath string) (string, error) {
	// Read file content
	...
	data := types.Data{
		...
		RSDataSlices:   3,             // Default values for Reed-Solomon
		RSParitySlices: 2,
	}
...
func deleteFile(kv *oukvStore.KV, hashInput string) error {
...
func restoreFile(kv *oukvStore.KV, hashInput string) error {
	key, err := resolveHashInput(kv, hashInput)
	if err != nil {
		return err
	}
...
func listStoredData(kv *oukvStore.KV) error {
...
func listStoredDataExtended(kv *oukvStore.KV) error {
	// Get detailed information about all stored data
	dataInfos, err := kv.ListStoredData()
	if err != nil {
		return fmt.Errorf("failed to list stored data: %w", err)
	...
	for i, info := range dataInfos {
		...
		var avgSize uint64
		if sliceCount > 0 {
			avgSize = totalSize / uint64(sliceCount)
		} else {
			minSize = 0 // Reset if no slices found
		}

		rsConfig := fmt.Sprintf("%d+%d", info.RSDataSlices, info.RSParitySlices)
...
func validateData(kv *oukvStore.KV) error {
